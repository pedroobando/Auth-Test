...
const Login = () => {
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      console.log(values);
      try {
        const { data, errors } = await (await authenticateRetToken(email, password)).json();
                  ...
                  <div className="relative w-full mb-3">
                    ...
                    {formik.touched.email && formik.errors.email && (
                      <div className="mb-2 mt-1 bg-red-50 border-l-4 border-red-500 text-red-700 px-4 py-2">
                        <p className="text-sm font-bold text-red-500">{formik.errors.email}</p>
                      </div>
                    )}
                  </div>
